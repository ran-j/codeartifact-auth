...
function setPackageType(input: string): packageTypes {
  try {
    ...
    throw exitMessage(
      "Unknown error when setting packageType",
...
function assignCommandArg(
  argType: string,
  value: string,
  commandArguments: CommandLineArguments
): string | void {
  if (argType === CmdTypeArgs.accountId)
    return (commandArguments.accountId = value);
  if (argType === CmdTypeArgs.domain) return (commandArguments.domain = value);
  if (argType === CmdTypeArgs.help) return (commandArguments.help = value);
  if (argType === CmdTypeArgs.region) return (commandArguments.region = value);
  if (argType === CmdTypeArgs.repository)
    return (commandArguments.repository = value);
  if (argType === CmdTypeArgs.packageType) return setPackageType(value);
  if (argType === CmdTypeArgs.scope) return (commandArguments.scope = value);
}

const findArgIndex = (args: string[], cmdArgTypes: string): number =>
  args.findIndex((arg) => cmdArgTypes === arg);

const findArgumentValue = (
  cmdArgTypes: string[],
  args: string[]
): string | undefined => {
  return cmdArgTypes
...
function needsHelp(help: string | undefined): void {
  if (help !== undefined) exitMessage(manpage, ExitCode.Successful);
}

const isUsingCmdArgs = (args: CommandLineArguments): boolean =>
  Object.values(args).filter((x) => x).length > 0;
...
function getCmdArgs(args: CommandLineArguments): awsCodeArtifactConfig {
  ...
  if (!accountId)
    throw exitMessage(
      `Missing ${CmdTypeArgs.accountId}. Please use ${cmdArgs.accountId}`,
      ExitCode.SyntaxError
    );

  if (!domain)
    throw exitMessage(
      `Missing ${CmdTypeArgs.domain} Please use ${cmdArgs.domain}`,
      ExitCode.SyntaxError
    );

  if (!region)
    throw exitMessage(
      `Missing ${CmdTypeArgs.region} Please use ${cmdArgs.region}`,
      ExitCode.SyntaxError
    );

  if (!repository)
    throw exitMessage(
      `Missing ${CmdTypeArgs.repository} Please use ${cmdArgs.repository}`,
      ExitCode.SyntaxError
    );

  if (!packageType)
    throw exitMessage(
      `Missing ${CmdTypeArgs.packageType} Please use ${cmdArgs.packageType}`,
      ExitCode.SyntaxError
    );
...
function getAwsConfig(args: CommandLineArguments): awsCodeArtifactConfig {
  if (isUsingCmdArgs(args)) return getCmdArgs(args);

  const packageJson = process.env.PACKAGE_FILE
    ? require(process.env.PACKAGE_FILE)
